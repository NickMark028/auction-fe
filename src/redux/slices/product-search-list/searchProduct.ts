...
export const searchProductTC = createAsyncThunk<TProductSearchList, TProductQuery, ThunkOption>(
    ...
    async function searchProduct(params, thunkAPI) {
        try {
            ...
            if (!keyword)
                return thunkAPI.rejectWithValue('keyword can not be empty');

            if (keyword.length < 4)
                return thunkAPI.rejectWithValue('keyword length must be at least 4 characters');
